...
def get_image_paths_and_labels(dataset):
    ...
    for i in range(len(dataset)):
        ...
        a = dataset[i].image_paths
        for j in range(len(a)):
            b = a[j].split('/')
            labels_flat.append(b[2])
...
def get_dataset(paths, has_class_directories=True):
    ...
    for path in paths.split(':'):
        ...
        for i in range(nrof_classes):
            ...
            if os.path.join(facedir,'.DS_Store') in image_paths:
                del(image_paths[(image_paths.index(os.path.join(facedir,'.DS_Store')))])
            # print(image_paths)
